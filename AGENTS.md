...
### Linting
⚠️ **No linter configured** - When adding linting, prefer ESLint with these rules:
...
ps.stdout.on('data', (data) => { /* handle output */ });
ps.stderr.on('data', (data) => { /* handle errors */ });
ps.on('close', (code) => { /* cleanup */ });
ps.on('error', (err) => { /* handle spawn failure */ });
```
